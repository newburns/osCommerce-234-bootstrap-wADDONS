        	...
        	if ($mail_error == false) {
          	...
          	if ($action == 'insert') {          		                   		
            	...
            	tep_db_perform(TABLE_MM_RESPONSEMAIL_RESTORE, $sql_data_array);
          		...
          		tep_redirect(tep_href_link(FILENAME_MM_RESPONSEMAIL, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'nID=' . $mail_id));
          		} else {
           			$action = 'new';       
         			}
      break;           
...
?>
	...
	<tr>
     ...
     <tr>
     	<td><?php echo tep_draw_form('mail', FILENAME_MM_RESPONSEMAIL, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'action=' . $form_action); if ($form_action == 'update') echo tep_draw_hidden_field('mail_id', $nID); ?>       
        ...
        <td>
        	<table border="0" width="100%" cellspacing="0" cellpadding="2">
          		<tr>
            		...
            		<?php echo (($form_action == 'insert') ? tep_image_submit('button_save.gif', IMAGE_SAVE) : tep_image_submit('button_update.gif', IMAGE_UPDATE)). '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . (isset($_GET['nID']) ? 'nID=' . $_GET['nID'] : '')) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?>
            		</td>
          		</tr>
        	</table>
        </td>
...
<?php
  ...
  if ( ($_GET['flag'] == '0') || ($_GET['flag'] == '1') ) {
          ...
          if (USE_CACHE == 'true') {
            ...
            echo '<tr><td class="main">Status changed to: <strong>'.$newstatus.'</strong><a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' .$_GET['nID']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a></td></tr>';    	
...
?>
      <tr><td align="right"><?php echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td></tr>
...
<?php
	...
	?>
    	...
    	<tr>
    		<td><?php echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button('button_back.gif', IMAGE_BACK); ?></a></td></tr>
...
<?php
    ...
    if ((!isset($_GET['nID']) || (isset($_GET['nID']) && ($_GET['nID'] == $mail['mail_id']))) && !isset($nInfo) && (substr($action, 0, 3) != 'new')) {
      ...
      if (isset($nInfo) && is_object($nInfo) && ($mail['mail_id'] == $nInfo->mail_id) ) {
       echo '<tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $nInfo->mail_id . '&action=preview') . '\'">' . "\n";      
      } else {
        echo '<tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $mail['mail_id']) . '\'">' . "\n";
      }
							?>             
                					<td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $mail['mail_id'] . '&action=preview') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $mail['title']; ?></td>
       						...
       						?>
                					...
                					<td class="dataTableContent" align="right"><?php if (isset($nInfo) && is_object($nInfo) && ($mail['mail_id'] == $nInfo->mail_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $mail['mail_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              					</tr>
	<?php
    	}
	?>
...
switch ($action) {
    ...
    case 'reset':
      ...
      $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $nInfo->mail_id . '&action='.$confirm_action) . '">' . tep_image_button('button_confirm.gif', 'confirm') . '</a>
      								<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
    break;
    default:
      if (is_object($nInfo)) {
        $heading[] = array('text' => '<b>' . $nInfo->title . '</b>');  
          $contents[] = array('align' => 'center', 'text' => '
          				<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $nInfo->mail_id . '&action=new') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> 
          				<a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $nInfo->mail_id . '&action=preview') . '">' . tep_image_button('button_preview.gif', IMAGE_PREVIEW) . '</a>           							       				
          				<table><tr><td class="main"><a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $nInfo->mail_id . '&action=restore') . '">' . tep_image_button('button_restore.gif', 'restore') . '</a>           				          				         				
          				</td></tr><tr><td class="main">'.TEXT_BACKUP_MESSAGE.'
          				</td></tr></table><table><tr><td class="main"><a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $nInfo->mail_id . '&action=reset') . '">' . tep_image_button('button_reset.gif', 'reset') . '</a>           				          				
          				</td></tr><tr><td class="main">'.TEXT_RESET_MESSAGE.'
          				</td></tr></table><table><tr><td class="main"><a href="' . tep_href_link(FILENAME_MM_RESPONSEMAIL, 'page=' . $_GET['page'] . '&nID=' . $nInfo->mail_id . '&action=test') . '">' . tep_image_button('button_send.gif', 'test') . '</a>
