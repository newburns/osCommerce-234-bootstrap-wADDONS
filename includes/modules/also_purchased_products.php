  ...
  if (isset($_GET['products_id'])) {
    ...
    if ($num_products_ordered >= MIN_DISPLAY_ALSO_PURCHASED) {
      ...
      while ($orders = tep_db_fetch_array($orders_query)) {
        ...
        $also_pur_prods_content .= '    <a href="' . tep_href_link('product_info', 'products_id=' . $orders['products_id']) . '">' . tep_image(DIR_WS_IMAGES . $orders['products_image'], $orders['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a>';
        $also_pur_prods_content .= '    <div class="caption">';
        $also_pur_prods_content .= '      <h5 class="text-center"><a href="' . tep_href_link('product_info', 'products_id=' . $orders['products_id']) . '">' . $orders['products_name'] . '</a></h5>';
