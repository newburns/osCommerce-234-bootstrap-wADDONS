...
// get some info from the coupon table
  $coupon_query=tep_db_query("select coupon_id, coupon_amount, coupon_type, coupon_minimum_order,uses_per_coupon, uses_per_user, restrict_to_products,restrict_to_categories from coupons where coupon_code='".$_POST['gv_redeem_code']."' and coupon_active='Y' and coupon_status = '1'");
  ...
  if ($coupon_result['coupon_type'] != 'G') {
    ...
    $date_query=tep_db_query("select coupon_start_date from coupons where coupon_start_date <= now() and coupon_code='".$_POST['gv_redeem_code']."'");
    ...
    $date_query=tep_db_query("select coupon_expire_date from coupons where coupon_expire_date >= now() and coupon_code='".$_POST['gv_redeem_code']."'");
    if (tep_db_num_rows($date_query)==0) {
      tep_redirect(tep_href_link('checkout_payment.php', 'error_message=' . urlencode(ERROR_INVALID_FINISDATE_COUPON), 'SSL'));
    }
    $coupon_count = tep_db_query("select coupon_id from coupon_redeem_track where coupon_id = '" . $coupon_result['coupon_id']."'");
    $coupon_count_customer = tep_db_query("select coupon_id from coupon_redeem_track where coupon_id = '" . $coupon_result['coupon_id']."' and customer_id = '" . $customer_id . "'");
    ...
    if (!isset($_SESSION['cc_id'])) tep_session_register('cc_id');
    ...
    if ($get_result['coupon_type'] != 'S') {
      ...
      if ($get_result['restrict_to_products'] || $get_result['restrict_to_categories']) {
        ...
        $valid_product = false;
          ...
          $cc_query = tep_db_query("select products_tax_class_id from products where products_id = '" . $t_prid . "'");
...
global $insert_id, $customer_id, $REMOTE_ADDR, $cc_id;
  if ($this->deduction !=0) {
    tep_db_query("insert into coupon_redeem_track (coupon_id, redeem_date, redeem_ip, customer_id, order_id) values ('" . $cc_id . "', now(), '" . $REMOTE_ADDR . "', '" . $customer_id . "', '" . $insert_id . "')");
  }
...
global $cart, $order;
  ...
  $product_query = tep_db_query("select products_id, products_price, products_tax_class_id, products_weight from products where products_id='" . $product_id . "'");
...
function check() {
  if (!isset($this->check)) {
    $check_query = tep_db_query("select configuration_value from configuration where configuration_key = 'MODULE_ORDER_TOTAL_COUPON_STATUS'");
...
function install() {
  ...
  tep_db_query("insert into configuration (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_COUPON_SORT_ORDER', '3', 'Sort order of display.', '6', '2', now())");
